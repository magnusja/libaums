...
private constructor(private val blockDevice: BlockDeviceDriver, private val fat: FAT, private val bootSector: Fat32BootSector,
                    private val entry: FatLfnDirectoryEntry, private var parent: FatDirectory?) : AbstractUsbFile() {
    ...
    override fun write(offset: Long, source: ByteBuffer) {
        initChain()
        var length = offset + source.remaining()
        if (length > length)
            length = length
    ...
    companion object {
         ...
         */
        @Throws(IOException::class)
        fun create(entry: FatLfnDirectoryEntry, blockDevice: BlockDeviceDriver,
                   fat: FAT, bootSector: Fat32BootSector, parent: FatDirectory): FatFile {
            return FatFile(blockDevice, fat, bootSector, entry, parent)
        }
    }
