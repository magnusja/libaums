 ...
 * @author mjahnen
 */
/* package */internal class FatDirectoryEntry {
    ...
    /* package */ val isShortNameLowerCase: Boolean
        get() = data!!.get(SHORTNAME_CASE_OFF) and 0x8.toByte() != 0

    /* package */ val isShortNameExtLowerCase: Boolean
        get() = data!!.get(SHORTNAME_CASE_OFF) and 0x10.toByte() != 0
     ...
     * @see .createLfnPart
     */
    /* package */ val isLfnEntry: Boolean
     ...
     */
    /* package */ val isVolumeLabel: Boolean
        get() = if (isLfnEntry)
            false
        else
     ...
     */
    /* package */ val isReadOnly: Boolean
     ...
     */
    /* package */ val isDeleted: Boolean
        get() = getUnsignedInt8(0) == ENTRY_DELETED

    /**
     * Returns the time this [.FatDirectoryEntry] was created.
     *
     * @return Time in milliseconds since January 1 00:00:00, 1970 UTC
     */
    /* package */
    /**
     * Sets the time this [.FatDirectoryEntry] was created.
     *
     * @param dateTime
     * Time in milliseconds since January 1 00:00:00, 1970 UTC
     */
    /* package */// TODO entry has also field which holds 10th seconds created
    ...
    /**
     * Returns the time this [.FatDirectoryEntry] was accessed the last
     * time.
     *
     * @return Time in milliseconds since January 1 00:00:00, 1970 UTC
     */
    /* package */
    /**
     * Sets the time this [.FatDirectoryEntry] was accessed the last
     * time.
     *
     * @param dateTime
     * Time in milliseconds since January 1 00:00:00, 1970 UTC
     */
    /* package */ var lastModifiedDateTime: Long
    ...
    /**
     * Returns the time this [.FatDirectoryEntry] was modified the last
     * time.
     *
     * @return Time in milliseconds since January 1 00:00:00, 1970 UTC
     */
    /* package */
    /**
     * Sets the time this [.FatDirectoryEntry] was modified the last
     * time.
     *
     * @param dateTime
     * Time in milliseconds since January 1 00:00:00, 1970 UTC.
     */
    /* package */ var lastAccessedDateTime: Long
     ...
     * @see .isVolumeLabel
     */
    /* package */ val volumeLabel: String
        get() {
    ...
    /* package */
    /**
     * Sets the start cluster for this [.FatDirectoryEntry]. The start
    ...
    /* package */ var startCluster: Long
        get() {
            ...
            return msb.toLong() shl 16 or lsb
        }
        set(newStartCluster) {
     ...
     * @see .setShortName
     */
    /* package */ fun getShortName(): ShortName? {
     ...
     * @see .getShortName
     */
    /* package */ fun setShortName(shortName: ShortName) {
     ...
     * @see .isLfnEntry
     */
    /* package */ fun extractLfnPart(builder: StringBuilder) {
    ...
    private fun getUnsignedInt8(offset: Int): Int {
        return data!!.get(offset) and 0xff
    }

    private fun getUnsignedInt16(offset: Int): Int {
        val i1 = data!!.get(offset) and 0xff
        val i2 = data!!.get(offset + 1) and 0xff
        return i2 shl 8 or i1
    }

    private fun getUnsignedInt32(offset: Int): Long {
        val i1 = (data!!.get(offset) and 0xff).toLong()
        val i2 = (data!!.get(offset + 1) and 0xff).toLong()
        val i3 = (data!!.get(offset + 2) and 0xff).toLong()
        val i4 = (data!!.get(offset + 3) and 0xff).toLong()
    ...
    companion object {
        ...
        /* package */ val ENTRY_DELETED = 0xe5
         ...
         * @see .setDirectory
         */
        /* package */ fun createNew(): FatDirectoryEntry {
         ...
         */
        /* package */ fun read(data: ByteBuffer): FatDirectoryEntry? {
         ...
         * @see .isVolumeLabel
         */
        /* package */ fun createVolumeLabel(volumeLabel: String): FatDirectoryEntry {
            val result = FatDirectoryEntry()
         ...
         * @see .isLfnEntry
         */
        /* package */ fun createLfnPart(unicode: String, offset: Int, checksum: Byte,
