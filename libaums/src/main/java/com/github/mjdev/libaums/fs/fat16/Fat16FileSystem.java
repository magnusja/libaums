...
public class Fat16FileSystem implements FileSystem {
    ...
    private static final String TAG = Fat16FileSystem.class.getSimpleName();
     ...
     * @param first512Bytes First 512 bytes read from block device.
     * @throws IOException If reading from the device fails.
     */
    private Fat16FileSystem(BlockDeviceDriver blockDevice, ByteBuffer first512Bytes) throws IOException {
        bootSector = Fat16BootSector.read(first512Bytes);
     ...
     * @throws IOException If reading from the device fails.
     */
    public static Fat16FileSystem read(BlockDeviceDriver blockDevice) throws IOException {

        ByteBuffer buffer = ByteBuffer.allocate(512);
        blockDevice.read(0, buffer);
        buffer.flip();

        if ((char) buffer.get(54) != 'F' ||
                (char) buffer.get(55) != 'A' ||
                (char) buffer.get(56) != 'T' ||
                (char) buffer.get(57) != '1' ||
                (char) buffer.get(58) != '6' ||
                (char) buffer.get(59) != ' ' ||
                (char) buffer.get(60) != ' ' ||
                (char) buffer.get(61) != ' ') {
            return null;
        ...
        return new Fat16FileSystem(blockDevice, buffer);
    }
